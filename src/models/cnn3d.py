#Sentinel-2とEmbeddingV1の両方を用いたデータセットでの3D-CNNモデル
...
ALPHA_AITCHISON = 1.0   # Aitchisonの割合（混合lossにしたいなら）
USE_HUBER_AITCH = True
HUBER_DELTA     = 0.5

# データ仕様

PATCH_SIZE  = 7         # 5/7/9でアブレーション
...
def valid_mask(X, Y):
    ...
    return np.isfinite(X).all(axis=0) & np.isfinite(Y).all(axis=0)
...
# ========= 3DパッチDataset =========
class AbundancePatch3DFromScaled(Dataset):
    def __init__(self, file_list, xmin, xmax, mode: str,
        ...
        for fi, tif in enumerate(file_list):
            X, Y, _ = load_XY_tif(tif)  # (64,H,W), (8,H,W)
            C, H, W = X.shape

            # --- スケーリング（S2のみmin-max、Embは無変換） ---
            Xs = scale_input_X(X, xmin, xmax, mode=self.mode, eps=1e-6)


            # --- valid mask ---
            Ym = np.moveaxis(Y, 0, -1)  # (H,W,8)
            ...
            for iy in range(self.half, H + self.half, center_stride):
                for ix in range(self.half, W + self.half, center_stride):
                    ...
                    if clamp_bounds:
                        ...
                        """
                    self.items.append(
                        (np.expand_dims(xpatch, 0).astype(np.float32),
...
class SpectralSpatial3DCNN(nn.Module):
    def __init__(self, n_classes=8, base=64, dropout=0.20):
        super().__init__()

        # 3Dのエッジ（隣接値）パディング
        self.pad = nn.ReplicationPad3d(1)

        self.conv1 = nn.Conv3d(1,      base,   kernel_size=3, padding=0, bias=False)
        self.gn1   = nn.GroupNorm(num_groups=8, num_channels=base)

        self.conv2 = nn.Conv3d(base,   base,   kernel_size=3, padding=0, bias=False)
        self.gn2   = nn.GroupNorm(num_groups=8, num_channels=base)

        self.conv3 = nn.Conv3d(base,   base*2, kernel_size=3, padding=0, bias=False)
        self.gn3   = nn.GroupNorm(num_groups=8, num_channels=base*2)

        self.conv4 = nn.Conv3d(base*2, base*2, kernel_size=3, padding=0, bias=False)
...
# ========= 学習（3D） =========
def train_with_validation_3d(train_ds, val_ds, V,
                             epochs, bs, lr, wd, device):
    model = SpectralSpatial3DCNN(
        n_classes=N_CLASSES, base=BASE_CH, dropout=DROPOUT
    ).to(device)
...
# ========= 予測TIF保存 =========
# ========= 評価（平均Aitchison） =========
def predict_single_image_metric_and_save_3d(tif_path, model, xmin, xmax, mode: str, V, eps, delta,
                                            patch=PATCH_SIZE, stride=CENTER_STRIDE_VAL, out_tif_path=None):
    X, Y, meta = load_XY_tif(tif_path)
    C, H, W = X.shape
    vm = valid_mask(X, Y)
    if not vm.any():
        return os.path.basename(tif_path), np.nan

    Xs = scale_input_X(X, xmin, xmax, mode=mode, eps=1e-6)

    half = patch // 2
    Xp = np.pad(Xs, ((0, 0), (half, half), (half, half)), mode="reflect")
    Yp = np.pad(Y, ((0, 0), (half, half), (half, half)), mode="reflect")
    ...
    if len(outs) == 0:
        return os.path.basename(tif_path), np.nan, np.nan, 0.0, 0
    ...
    zp = ilr_numpy(outs_np, V_np, eps=eps)
    zt = ilr_numpy(Ymr,    V_np, eps=eps)
    d2 = ((zp - zt) ** 2).sum(axis=1)
    ...
    return os.path.basename(tif_path), mean_ait, mrmse, mae, d2.sum(), d2.size

def predict_images_metric_and_save_3d(files, model, xmin, xmax, mode: str, V, eps, delta,
    ...
    for p in files:
        ...
        image_name, mean_ait, mrmse, mae, d2_sum, count = predict_single_image_metric_and_save_3d(
            p, model, xmin, xmax, mode, V, eps, delta, patch, stride, out_tif_path
        )

        rows.append({"image": image_name, "mean_aitchison_ilr": mean_ait, "mrmse": mrmse, "mae": mae})

        if not np.isnan(mean_ait):
            total_d2_sum += d2_sum
            total_count += count
...
def main():
    ...
    # Train
    model, best_val, best_ep = train_with_validation_3d(
        train_ds, val_ds, V,
        epochs=EPOCHS, bs=BATCH_SIZE_3D, lr=LR, wd=WEIGHT_DECAY,
        device=DEVICE
    )

    print(f"[Best] epoch={best_ep}  val_aitchison={best_val:.6f}")

    # Val/Test 評価（平均Aitchison）
    rows_val,  val_global_mean, val_global_mrmse, val_global_mae = predict_images_metric_and_save_3d(
    ...
    # ---- Test画像の予測TIFを保存（test 全画像）----
    preds_dir = os.path.join(run_dir, "preds")
    os.makedirs(preds_dir, exist_ok=True)

    print(f"[Pred] saving predictions for {len(test_files)} test images ...")

    # 統合関数でmetric計算と保存を同時に（1回の推論で両方）
    rows_test, test_global_mean, test_global_mrmse, test_global_mae = predict_images_metric_and_save_3d(
    ...
    with open(os.path.join(run_dir,"config.json"),"w",encoding="utf-8") as f:
        json.dump({
            ...
            "train_ratio":TRAIN_RATIO,"batch_size":BATCH_SIZE_3D,
